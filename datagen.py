...
class DataGenerator():
	...
	"""
	def __init__(self, joints_name = None, img_dir=None, train_data_file = None, remove_joints = None, keep_invalid = False):
		""" Initializer
		Args:
		...
		for line in input_file:
			...
			if joints == [-1] * len(joints):
				...
				if not self.keep_invalid:
					joints[np.equal(w, 0)] = [-1, -1]
				#for i in range(joints.shape[0]):
					#if np.array_equal(joints[i], [-1,-1]):
						#w[i] = 0
				self.data_dict[name] = {'box' : box, 'joints' : joints, 'weights' : w, 'category': category}
	...
	def _generate_hm(self, height, width ,joints, maxlenght, weight):
		...
		new_j = joints.astype(np.int32)
		for i in range(num_joints):
			if not(np.array_equal(new_j[i], [-1,-1])) and weight[i] == 1:
				s = int(np.sqrt(maxlenght) * maxlenght * 10 / 4096) + 2
				hm[:,:,i] = self._makeGaussian(height, width, sigma= s, center= (new_j[i,0], new_j[i,1]))
			else:
